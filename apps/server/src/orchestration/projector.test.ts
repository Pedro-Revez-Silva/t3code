...
describe("orchestration projector", () => {
  ...
  it("settles a running latest turn when a session becomes idle", async () => {
    const createdAt = "2026-02-23T08:00:00.000Z";
    const startedAt = "2026-02-23T08:00:05.000Z";
    const finishedAt = "2026-02-23T08:00:09.000Z";
    const model = createEmptyReadModel(createdAt);
    ...
    const afterIdle = await Effect.runPromise(
      projectEvent(
        afterRunning,
        makeEvent({
          sequence: 3,
    ...
    const thread = afterIdle.threads[0];
