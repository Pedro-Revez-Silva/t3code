...
const makeOrchestrationEngine = Effect.gen(function* () {
  ...
  const getReadModel: OrchestrationEngineShape["getReadModel"] = () =>
    Effect.sync((): OrchestrationReadModel => commandReadModel);

  const refreshReadModel: OrchestrationEngineShape["refreshReadModel"] = () =>
    projectionSnapshotQuery.getCommandReadModel().pipe(
      Effect.catch(() => Effect.succeed(commandReadModel)),
      Effect.tap((snapshot) =>
        Effect.sync(() => {
          commandReadModel = snapshot;
        }),
      ),
    );
