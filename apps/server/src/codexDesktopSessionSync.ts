...
function makeProjectId(workspaceRoot: string): ProjectId {
  ...
  return ProjectId.makeUnsafe(`codex-project-${digest.slice(0, 16)}`);
}
...
function makeMessageId(threadId: ThreadId, ordinal: number): MessageId {
  return MessageId.makeUnsafe(`codex-msg-${threadId}-${ordinal}`);
}

function makeActivityId(threadId: ThreadId, ordinal: number): EventId {
  return EventId.makeUnsafe(`codex-activity-${threadId}-${ordinal}`);
}
...
function resolveInteractionMode(payload: Record<string, unknown>): ProviderInteractionMode | undefined {
...
function parseSessionFile(
  ...
  for (const line of lines) {
    ...
    if (type === "session_meta") {
      ...
      threadId = ThreadId.makeUnsafe(rawThreadId);
    ...
    if (eventType === "task_started") {
      ...
      if (nextTurnId) {
        latestTurnId = TurnId.makeUnsafe(nextTurnId);
    ...
    if (eventType === "task_complete") {
      ...
      if (completedTurnId) {
        latestTurnId = TurnId.makeUnsafe(completedTurnId);
      }
      activeTurnId = null;
      continue;
    }
...
> = Effect.gen(function* () {
  ...
  const syncOnce = Effect.gen(function* () {
    ...
    yield* Ref.set(syncState, {
      sessionIndexMtimeMs: sessionIndexUpdated ? statSync(sessionIndexPath).mtimeMs : nextSessionIndexMtimeMs,
      sessionFileMtimes: fileStats,
    });
  ...
  yield* syncOnce.pipe(
    Effect.flatMap((changed) => (changed ? orchestrationEngine.refreshReadModel() : Effect.void)),
    Effect.catch((cause) =>
    ...
    Effect.forever(
      syncOnce.pipe(
        Effect.flatMap((changed) =>
          changed ? orchestrationEngine.refreshReadModel() : Effect.void,
        ),
        Effect.catch((cause) =>
